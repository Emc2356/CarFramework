...
// CR_FORCE_INLINE std::string compileSingleShader(const std::string& path,
...
//     compiler.CompileGlslToSpv(sourceCode, kind, path.c_str(), options); if
...
namespace Car {
    Ref<Shader> Shader::Create(const std::string& vertexShaderName, const std::string& fragmeantShaderName) {
        return Shader::CreateImpl(readFile("./resources/shaders/" + vertexShaderName),
