...
namespace Car {
    ...
    void VulkanRenderer::InitImpl() {
        sGraphicsContext = reinterpretCastRef<VulkanGraphicsContext>(Application::Get()->getWindow()->getGraphicsContext());
    ...
    void VulkanRenderer::BeginRecordingImpl() {
        ...
        VkCommandBuffer cmdBuffer = sGraphicsContext->getCurrentCommandBuffer();
    ...
    void VulkanRenderer::DrawTrianglesImpl(const Ref<VertexArray> va, uint32_t triangleCount) {
        ...
        vkCmdDraw(cmdBuffer, triangleCount * 3, 1, 0, 0);
    }
