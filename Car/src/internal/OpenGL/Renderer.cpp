...
namespace Car {
    ...
    void OpenGLRenderer::ClearColorImpl(float r, float g, float b, float a) { glClearColor(r, g, b, a); }

    void OpenGLRenderer::ClearImpl() { glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); }
    ...
    void OpenGLRenderer::DisableBlendingImpl() { glDisable(GL_BLEND); }

    void OpenGLRenderer::EnableDepthTestImpl() { glEnable(GL_DEPTH_TEST); }

    void OpenGLRenderer::DisableDepthTestImpl() { glDisable(GL_DEPTH_TEST); }
    ...
    void OpenGLRenderer::DrawTrianglesImpl(const Ref<VertexArray> va, uint32_t triangleCount) {
        ...
        glDrawElements(GL_TRIANGLES, triangleCount * 3,
                       ((OpenGLIndexBuffer*)(va->getIndexBuffer().get()))->getOpenGLType(), nullptr);
    }
