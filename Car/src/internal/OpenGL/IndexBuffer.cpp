...
namespace Car {
    OpenGLIndexBuffer::OpenGLIndexBuffer(void* data, uint32_t count, Buffer::Usage usage, Buffer::Type type) {
        ...
        default: {
            CR_CORE_ERROR("Unrecognized Usage for IndexBuffer: {}", static_cast<uint32_t>(usage));
            CR_DEBUGBREAK();
        }
        }
    ...
    OpenGLIndexBuffer::~OpenGLIndexBuffer() { glDeleteBuffers(1, &mID); }

    void OpenGLIndexBuffer::bind() const { glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mID); }
    ...
    void OpenGLIndexBuffer::updateData(void* data, uint32_t count, uint32_t offset) {
        ...
        if (count <= mCount) {
            ...
            if (offset == 0) {
                ...
                glBufferData(GL_ELEMENT_ARRAY_BUFFER, (count + offset) * Buffer::sizeOfType(mType), mapped,
                             mOpenGLUsage);
        ...
        glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, offset * Buffer::sizeOfType(mType), count * Buffer::sizeOfType(mType),
                        data);
    }
