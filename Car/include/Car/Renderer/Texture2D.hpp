...
namespace Car {
    ...
    public:
        ...
        static Ref<Texture2D> Create(const std::string& filepath, bool flipped = false, Format format = Format::RGBA,
                                     Format internalFormat = Format::RGBA8, Type type = Type::Byte);
        static Ref<Texture2D> Create(uint32_t width, uint32_t height, uint8_t* buffer = nullptr,
                                     Format format = Format::RGBA, Format internalFormat = Format::RGBA8,
                                     Type type = Type::Byte);
    };
} // namespace Car
