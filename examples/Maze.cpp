    ...
    void draw(const glm::ivec2& cellSize) {
        if (!visited) {
            Car::Renderer2D::DrawRect(
                {float(i * cellSize.x), float(j * cellSize.y), float(cellSize.x), float(cellSize.y)},
                {1.0f, 1.0f, 1.0f});
        } else {
            Car::Renderer2D::DrawRect(
                {float(i * cellSize.x), float(j * cellSize.y), float(cellSize.x), float(cellSize.y)},
        ...
        if (wallMask & TOP) {
            Car::Renderer2D::DrawLine({float(i * cellSize.x), float(j * cellSize.y)},
                                      {float(i * cellSize.x + cellSize.x), float(j * cellSize.y)}, {0.0f, 0.0f, 0.0f});
        }
        if (wallMask & LEFT) {
            Car::Renderer2D::DrawLine({float(i * cellSize.x), float(j * cellSize.y)},
                                      {float(i * cellSize.x), float(j * cellSize.y + cellSize.y)}, {0.0f, 0.0f, 0.0f});
        }
        if (wallMask & BOTTOM) {
            Car::Renderer2D::DrawLine({float(i * cellSize.x), float(j * cellSize.y + cellSize.y)},
                                      {float(i * cellSize.x + cellSize.x), float(j * cellSize.y + cellSize.y)},
                                      {0.0f, 0.0f, 0.0f});
        }
        if (wallMask & RIGHT) {
            Car::Renderer2D::DrawLine({float(i * cellSize.x + cellSize.x), float(j * cellSize.y)},
                                      {float(i * cellSize.x + cellSize.x), float(j * cellSize.y + cellSize.y)},
                                      {0.0f, 0.0f, 0.0f});
        }
    }
